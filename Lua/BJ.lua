	...
	--===========================================================================
	-- Runs the  |s actions if the  |s conditions evaluate to true.
	--
	...
	--===========================================================================
	-- Arranges for a   to fire almost immediately, except that the calling
	--   is not interrupted as is the case with a TriggerExecute .
	-- Since the   executes normally, its conditions are still evaluated.
	--
	...
	--===========================================================================
	-- Debug - Display the contents of the   queue (as either nil or "x"
	-- for each entry).
	...
	--===========================================================================
	-- Searches the queue for a given  , returning the index of the
	--   within the queue if it is found, or -1 if it is not found.
	--
	...
	--===========================================================================
	-- Removes a   from the   queue, shifting other triggers down
	-- to fill the unused space.  If the currently running   is removed
	-- in this manner, this function does NOT attempt to run the next  .
	--
	...
	--===========================================================================
	-- Attempt to execute the first   in the queue.  If it fails, remove
	-- it and execute the next one.  Continue this cycle until a   runs,
	...
	--===========================================================================
	-- Queues a   to be executed, assuring that such triggers are not
	...
	--===========================================================================
	-- Denotes the end_jass of a queued  . Be sure to  this only once per
	-- queued  , or risk stepping on the toes of other queued triggers.
	--
	function QueuedTriggerRemoveBJ (   trig )  
		...
		-- Shuffle the other   entries down to fill in the gap.
		 QueuedTriggerRemoveByIndex(trigIndex)

		-- If we just axed the currently running  , run the next one.
		if (trigIndex == 0) then
	...
	--===========================================================================
	-- Denotes the end_jass of a queued  . Be sure to  this only once per
	-- queued  , lest you step on the toes of other queued triggers.
	--
	function QueuedTriggerDoneBJ (   )  
		...
		-- Remove the currently running   from the . = {}
		 QueuedTriggerRemoveByIndex(0)
	...
	--===========================================================================
	-- Remove all but the currently executing   from the   queue.
	--
	...
	--===========================================================================
	-- We can|t do game-time waits, so this simulates one by starting a  
	-- and polling until the   expires.
	...
	--*  These functions exist purely to make the   dialogs cleaner and
	--*  more comprehensible.
	--*
	--***************************************************************************
	...
	--===========================================================================
	-- This function ) the input  , converting it from the localized text, if necessary
	--
	...
	--===========================================================================
	-- Returns a square   that exactly encompasses the specified circle.
	--
	...
	--
	function SubStringBJ (   source,   start,   end_jass )  
		return SubString(source, start-1, end_jass)
	end  
	...
	function SetAmbientDaySound (   inLabel )  
		...
		-- Start the   if necessary, based on current time
		 ToD = GetTimeOfDay()
	...
	--===========================================================================
	-- Two distinct   actions can|t share the same function name, so this
	-- dummy simply() mimics the behavior of an existing .
	--
	...
	--===========================================================================
	-- Picks a random   from within a  , matching a condition
	--
	...
	--===========================================================================
	-- Game   explicitly sets the animation back to "decay bone" after the
	...
	function DelayedSuspendDecay (   )  
		...
		-- Switch the global   groups over to local variables and recreate
		-- the global versions, so that this function can   overlapping
		-- calls.
	...
	--===========================================================================
	-- Returns true if every   of the   is dead.
	--
	function IsUnitGroupDeadBJ (   g )  
		-- If the user wants the   destroyed, remember that fact and clear
		...
		-- If the user wants the   destroyed, do so now.
		if (wantDestroy) then
			 DestroyGroup(g)
		end
	...
	--===========================================================================
	-- Returns true if the   contains no units.
	--
	...
	--===========================================================================
	-- Returns true if every   of the   is within the given  .
	--
	...
	function IssueHauntOrderAtLocBJ (   whichPeon,   loc )  
		...
		-- Search for a gold mine within a 1-cell radius of the specified  .
		 g = CreateGroup()
	...
	function PauseAllUnitsBJ (   pause )  
		...
		while true do
			...
			-- Enumerate and unpause every   owned by the  .
	...
	--===========================================================================
	-- This attempts to replace a   with a new   type by creating a new
	--   of the desired type using the old  |s  , facing, etc.
	--
	function ReplaceUnitBJ (   whichUnit,   newUnitId,   unitStateMethod )  
		...
		-- Set the  |s life and mana according to the requested method.
		if (unitStateMethod == bj_UNIT_STATE_METHOD_RELATIVE) then
			-- Set the replacement|s current/max life ratio to that of the old  .
		...
		elseif (unitStateMethod == bj_UNIT_STATE_METHOD_ABSOLUTE) then
			-- Set the replacement|s current life to that of the old  .
			-- If the new   has mana, do the same for mana.
		...
		elseif (unitStateMethod == bj_UNIT_STATE_METHOD_DEFAULTS) then
			-- The newly created   should already have default life and mana.
		elseif (unitStateMethod == bj_UNIT_STATE_METHOD_MAXIMUM) then
			...
			-- Unrecognized   state method - ignore the request.
		end

		-- Mirror properties of the old   onto the new  .
		-- PauseUnit(newUnit, IsUnitPaused(oldUnit))
		 SetResourceAmount(newUnit, GetResourceAmount(oldUnit))

		-- If both the old and new units are heroes,   their hero info.
		...
		-- Remove or kill the original  .  It is sometimes unsafe to remove
		-- hidden units, so kill the original   if it was previously hidden.
		if wasHidden then
			 KillUnit(oldUnit)
			 RemoveUnit(oldUnit)
		else
	...
	--===========================================================================
	-- Enumerates within a  , with a filter to narrow the enumeration down
	...
	-- want to change to, but also what height we are currently at.  This  
	...
	--===========================================================================
	-- Grab the   and throw his own coords in his face, forcing him to push
	...
	--===========================================================================
	-- Nudge the items and units within a given   ever so gently, so as to
	...
	-- or reviving a pathing blocker at the appropriate   (and creating
	...
	function ChangeElevatorWallBlocker (   x,   y,   facing,   open )  
		...
		elseif (GetDestructableTypeId(blocker) ~= bj_ELEVATOR_BLOCKER_CODE) then
			...
			-- the request.  (Two destructibles cannot occupy the same  
	...
	--===========================================================================
	-- Consider each  , one at a time, keeping a "current pick".   Once all units
	-- are considered, this "current pick" will be the resulting random  .
	--
	-- The chance of picking a given   over the "current pick" is 1/N, where N is
	...
	function GetRandomSubGroupEnum (   )  
		if (bj_randomSubGroupWant > 0) then
			if (bj_randomSubGroupWant >= bj_randomSubGroupTotal) or (GetRandomReal(0,1) < bj_randomSubGroupChance) then
				-- We either need every remaining  , or the   passed its chance check.
	...
	--===========================================================================
	-- This   differs from the common.j interface in that the alpha value
	...
	--===========================================================================
	-- Sets a  |s facing to point directly at a  .
	--
	...
	--===========================================================================
	-- Sets a  |s facing to point directly at another  .
	--
	...
	function SetPlayerAllianceStateBJ (   sourcePlayer,   otherPlayer,   allianceState )  
		...
		elseif allianceState == bj_ALLIANCE_NEUTRAL_VISION then
			...
			-- Unrecognized   state - ignore the request.
		end
	end

	--===========================================================================
	-- Set the   states for an entire   towards another  .
	--
	...
	--===========================================================================
	-- Force (whichPlayer) AI   to share vision and advanced   control 
	...
	--===========================================================================
	-- Force (whichPlayer) to share vision and advanced   control with all of his/her allies.
	--
	...
	--===========================================================================
	-- Creates a |Neutral Victim|   slot.  This slot is passive towards all
	...
	--===========================================================================
	-- Change ownership for every   of (whichPlayer)|s team to neutral passive.
	--
	...
	function MeleeVictoryDialogBJ (   whichPlayer,   leftGame )  
		...
		-- Display "  was victorious" or "  has left the game" message
		if (leftGame) then
	...
	function MeleeDefeatDialogBJ (   whichPlayer,   leftGame )  
		...
		-- Display "  was defeated" or "  has left the game" message
		if (leftGame) then
			 formatString = GetLocalizedString( "PLAYER_LEFT_GAME" )
		else
		...
		-- Only show the continue   if the game is not over and observers on death are allowed
	...
	--===========================================================================
	-- Returns the   who is occupying a specified position in a  .
	...
	--===========================================================================
	-- If cancelled, stop the   and end_jass the cinematic scene.
	--
	...
	--===========================================================================
	-- Init a   to listen for END_CINEMATIC events and respond to them if
	...
	function WaitTransmissionDuration (   soundHandle,   timeType,   timeVal )  
		...
		elseif (soundHandle == nil) then
			-- If the   does not exist, perform a default length wait.
			 TriggerSleepAction(bj_NOTHING_SOUND_DURATION)

		elseif (timeType == bj_TIMETYPE_SUB) then
			-- If the transmission is cutting off the  , wait for the  
			-- to be mostly finished.
			 WaitForSoundBJ(soundHandle, timeVal)

		elseif (timeType == bj_TIMETYPE_ADD) then
			-- If the transmission is extending beyond the  |s length, wait
	...
	--===========================================================================
	-- Display a text message to a Player Group with an accompanying  ,
	...
	--   - Display a speech indicator for the  
	--   - Ping the minimap
	--
	function TransmissionFromUnitWithNameBJ (   toForce,   whichUnit,   unitName,   soundHandle,   message,   timeType,   timeVal,   wait )  
		...
		if (IsPlayerInForce(GetLocalPlayer(), toForce)) then
			...
			if (whichUnit == nil) then
				-- If the   reference is invalid, send the transmission from the center of the map with no portrait.
	...
	--===========================================================================
	-- This operates like TransmissionFromUnitWithNameBJ, but for a   type
	-- rather than a   instance.  As such, no speech indicator is employed.
	--
	...
	function FinishCinematicFadeAfterBJ (   duration )  
		-- Create a   to end_jass the cinematic fade.
	...
	function ContinueCinematicFadeAfterBJ (   duration,   red,   green,   blue,   trans,   tex )  
		...
		-- Create a   to continue the cinematic fade.
	...
	--===========================================================================
	-- Rescues a   for a  .  This performs the default rescue behavior,
	-- including a rescue  , flashing selection circle, ownership change,
	-- and optionally a   color change.
	--
	...
	-- reasons, this should only be attempted if a   is  to Rescuable,
	-- or if a specific   is thus flagged.
	--
	...
	function MakeUnitRescuableToForceBJ (   whichUnit,   isRescuable,   whichForce )  
		-- Flag the   as rescuable/unrescuable for the appropriate players.
	...
	function InitRescuableBehaviorBJ (   )  
		local   index
		 ...
		 index = 0
		while true do
			-- If at least one   slot is "Rescuable"-controlled, init the
	...
	-- so that the   for this can use a single drop-down to list all of
	...
	function AdjustPlayerStateBJ (   delta,   whichPlayer,   whichPlayerState )  
		-- If the change was positive, apply the difference to the  |s
	...
	--===========================================================================
	-- Replaces a gold mine with a blighted gold mine for the given  .
	--
	function BlightGoldMineForPlayerBJ (   goldMine,   whichPlayer )  
		...
		-- Make sure we|re replacing a Gold Mine and not some other type of  .
	...
	--===========================================================================
	-- Utility function for use by editor-generated   drop table triggers.
	-- This function is added as an action to all   drop triggers,
	-- so that a   drop may be differentiated from a   drop.
	--
	...
	function MeleeStartingHeroLimit (   )  
		...
		while true do
			...
			-- each   is restricted to a limit per hero type as well
	...
	--===========================================================================
	-- The first N heroes trained or hired for each   start off with a
	...
	function MeleeGrantItemsToHero (   whichUnit )  
		...
		-- If we haven|t twinked N heroes for this   yet, twink away.
	...
	function MeleeGrantHeroItems (   )  
		...
		-- Register for an   whenever a hero is trained, so that we can give
		...
		-- Register for an   whenever a neutral hero is hired, so that we
		...
		-- starting units   can create them as necessary.
	...
	function MeleeClearExcessUnits (   )  
		...
		while true do
			...
			-- If the   slot is being used, clear any nearby creeps.
			if (GetPlayerSlotState(indexPlayer) == PLAYER_SLOT_STATE_PLAYING) then
	...
	function MeleeRandomHeroLoc (   p,   id1,   id2,   id3,   id4,   loc )  
		...
		-- The selection of heroes is dependant on the game  .
		 v = VersionGet()
		if (v == VERSION_REIGN_OF_CHAOS) then
	...
	--===========================================================================
	-- Returns a   which is (distance) away from (src) in the direction of (targ).
	--
	...
	--   - 1 Town Hall, placed at start  
	--   - 5 Peasants, placed between start   and nearest gold mine
	--
	function MeleeStartingUnitsHuman (   whichPlayer,   startLoc,   doHeroes,   doCamera,   doPreload )  
		...
		if (nearestMine ~= nil) then
			...
			-- Set random hero spawn point to be off to the side of the start  .
			...
			-- Set random hero spawn point to be just south of the start  .
		...
		if (doHeroes) then
			-- If the "Random Hero" option is , start the   with a random hero.
	...
	--   - 1 Great Hall, placed at start  
	--   - 5 Peons, placed between start   and nearest gold mine
	--
	...
	--   - 1 Necropolis, placed at start  
	--   - 1 Haunted Gold Mine, placed on nearest gold mine
	--   - 3 Acolytes, placed between start   and nearest gold mine
	--   - 1 Ghoul, placed between start   and nearest gold mine
	...
	function MeleeStartingUnitsUndead (   whichPlayer,   startLoc,   doHeroes,   doCamera,   doPreload )  
		...
		if (doPreload) then
		 ...
		 nearestMine = MeleeFindNearestMine(startLoc, bj_MELEE_MINE_SEARCH_RADIUS)
		if (nearestMine ~= nil) then
			-- Spawn Necropolis at the start  .
			 CreateUnitAtLoc(whichPlayer, |unpl|, startLoc, bj_UNIT_FACING)
			
			-- Replace the nearest gold mine with a blighted  .
			...
			-- Create a patch of blight around the start  .
	...
	function MeleeStartingUnitsNightElf (   whichPlayer,   startLoc,   doHeroes,   doCamera,   doPreload )  
		...
		if (nearestMine ~= nil) then
			...
			-- Spawn Tree of Life at the start  .
	...
	--   - 12 Sheep, placed randomly around the start  
	--
	...
	function MeleeStartingUnits (   )  
		...
		while true do
			...
			if (GetPlayerSlotState(indexPlayer) == PLAYER_SLOT_STATE_PLAYING) then
				...
				-- Create initial  -specific starting units
	...
	function MeleeStartingUnitsForPlayer (   whichRace,   whichPlayer,   loc,   doHeroes )  
		...
		elseif (whichRace == RACE_NIGHTELF) then
			...
			-- Unrecognized   - ignore the request.
		end
	end
	...
	function MeleeStartingAI (   )  
		...
		while true do
			...
			if (GetPlayerSlotState(indexPlayer) == PLAYER_SLOT_STATE_PLAYING) then
				 indexRace = GetPlayerRace(indexPlayer)
				if (GetPlayerController(indexPlayer) == MAP_CONTROL_COMPUTER) then
					-- Run a  -specific melee AI script.
					if (indexRace == RACE_HUMAN) then
	...
	function MeleePlayerIsOpponent (   playerIndex,   opponentIndex )  
		...
		-- The   himself is not an opponent.
		if (playerIndex == opponentIndex) then
			return false
		end

		-- Unused   slots are not opponents.
		...
		-- Allied players with allied victory  are not opponents.
	...
	--===========================================================================
	-- Count buildings currently owned by all allies, including the   themself.
	--
	...
	--===========================================================================
	-- Count allies, excluding dead players and the   themself.
	--
	...
	--===========================================================================
	-- Counts key structures owned by a   and his or her allies, including
	...
	function MeleeRemoveObservers (   )  
		...
		-- Give all observers the game over  
		 playerIndex = 0
		while true do
			 indexPlayer = Player(playerIndex)
	...
	-- remaining players.  If even one   is not allied towards another,
	...
	function MeleeCheckForVictors (   )  
		...
		while true do
			if (not bj_meleeDefeated[playerIndex]) then
				-- Determine whether or not this   has any remaining opponents.
				 opponentIndex = 0
				while true do
				...
				-- Keep track of each opponentless   so that we can give
	...
	--===========================================================================
	-- Test each   to determine if anyone has been defeated.
	--
	function MeleeCheckForLosersAndVictors (   )  
		...
		-- Check each   to see if he or she has been defeated yet.
		 playerIndex = 0
		while true do
			 indexPlayer = Player(playerIndex)
			...
			if (not bj_meleeDefeated[playerIndex] and not bj_meleeVictoried[playerIndex]) then
				...
				if (MeleeGetAllyStructureCount(indexPlayer) <= 0) then
					...
					-- Keep track of each defeated   so that we can give
					-- them a defeat later.
		...
		-- Now that the defeated flags are , check if there are any victors
	...
	--===========================================================================
	-- Returns a  -specific "build X or be revealed" message.
	--
	...
	--===========================================================================
	-- Returns a  -specific "build X" label for cripple timers.
	--
	...
	--===========================================================================
	-- Test each   to determine if anyone has become crippled.
	--
	function MeleeCheckForCrippledPlayers (   )  
		...
		-- Check each   to see if he or she has been crippled or uncrippled.
		 playerIndex = 0
		while true do
			 indexPlayer = Player(playerIndex)
			...
			if (not bj_playerIsCrippled[playerIndex] and isNowCrippled) then
				...
				-- Player became crippled; start their cripple  .
			...
			elseif (bj_playerIsCrippled[playerIndex] and not isNowCrippled) then
				...
				-- Player became uncrippled; stop their cripple  .
				...
				if (GetLocalPlayer() == indexPlayer) then
					...
					-- Display a confirmation message if the  |s team is still alive.
					if (MeleeGetAllyStructureCount(indexPlayer) > 0) then
				...
				-- If the   granted shared vision, deny that vision now.
	...
	--===========================================================================
	-- Determine if the lost   should result in any defeats or victories.
	--
	function MeleeCheckLostUnit (   lostUnit )  
		...
		-- Check if the lost   has crippled or uncrippled the  .
		-- (A team with 0 units is dead, and thus considered uncrippled.)
		 MeleeCheckForCrippledPlayers()
	end

	--===========================================================================
	-- Determine if the gained   should result in any defeats, victories,
	-- or cripple-status changes.
	--
	function MeleeCheckAddedUnit (   addedUnit )  
		local   addedUnitOwner = GetOwningPlayer(addedUnit)

		-- If the   was crippled, this   may have uncrippled him/her.
	...
	function MeleeTriggerActionPlayerLeft (   )  
		...
		-- This is the same as defeat except the   generates the message 
		-- "  left the game" as opposed to "  was defeated".
	...
	function MeleeTriggerTournamentFinishSoon (   )  
		-- Note: We may get this   multiple times
		local      playerIndex
		local       indexPlayer
		...
		if not bj_finishSoonAllExposed then
			...
			-- Reset all crippled players and their timers, and hide the local crippled    
			 playerIndex = 0
			while true do
				 indexPlayer = Player(playerIndex)
		...
		-- Show the "finish soon"     and  the   time remaining
	...
	function MeleeInitVictoryDefeat (   )  
		...
		while true do
			...
			if (GetPlayerSlotState(indexPlayer) == PLAYER_SLOT_STATE_PLAYING) then
				...
				-- Set a   to fire whenever a building is cancelled for this  .
				 trig = CreateTrigger()
				...
				-- Set a   to fire whenever a   dies for this  .
				 trig = CreateTrigger()
				...
				-- Set a   to fire whenever a   begins construction for this  
				 trig = CreateTrigger()
	...
	function UpdateStockAvailability (   whichItem )  
		...
		elseif (iType == ITEM_TYPE_ARTIFACT) then
			...
			-- Not interested in this   type - ignore the  .
		end
	end

	--===========================================================================
	-- Find a sellable   of the given type and level, and then add it.
	--
	...
	function PerformStockUpdates (   )  
		...
		-- Make sure we found a valid   type to add.
	...
	function InitNeutralBuildings (   )  
		...
		-- Set up a   to fire whenever an   is sold.
	...
	function InitBlizzard (   )  
		-- Set up the Neutral Victim   slot, to torture the abandoned units
		-- of defeated players.  Since some triggers expect this   slot to
	...
	--*  Drop  
	--*
	--*  Makes the given   drop the given  
	--*
	--*  Note: This could potentially cause problems if the   is standing
	--*        right on the edge of an unpathable area and happens to drop the
	--*          into the unpathable area where nobody can get it...
	--*
	--***************************************************************************
